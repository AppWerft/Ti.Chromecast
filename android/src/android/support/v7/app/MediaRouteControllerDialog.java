...
public class MediaRouteControllerDialog extends AlertDialog {
	...
	// Time to wait before updating the volume when the user lets go of the seek
	// bar
	// to allow the route provider time to propagate the change and publish a
	// new
	// route descriptor.
	...
	/**
	 * Provides the subclass an opportunity to create a view that will be
	 * included within the body of the dialog to offer additional media controls
	 ...
	 * {@link #onCreateMediaControlView(Bundle)}.
	 *
	 * @return The media control view, or null if none.
	 */
	...
	/**
	 * Sets whether to enable the volume slider and volume control using the
	...
	/**
	 * Returns whether to enable the volume slider and volume control using the
	 * volume keys when the route supports it.
	 */
	...
	private void setMediaSession(MediaSessionCompat.Token sessionToken) {
		...
		try {
			mMediaController = new MediaControllerCompat(getContext(),
					sessionToken);
		...
		mState = mMediaController == null ? null : mMediaController
				.getPlaybackState();
		update();
	}
	...
	public MediaSessionCompat.Token getMediaSession() {
		return mMediaController == null ? null : mMediaController
				.getSessionToken();
	}
		...
		mVolumeSlider
				.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
					...
					public void onStopTrackingTouch(SeekBar seekBar) {
						// Defer resetting mVolumeSliderTouched to allow the
						// media route provider
						// a little time to settle into its new state and
						// publish the final
						// volume update.
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
			...
			mRoute.requestUpdateVolume(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN ? -1
					: 1);
			return true;
		}
	...
	private boolean update() {
		...
		if (mControlView == null) {
			...
			CharSequence title = mDescription == null ? null : mDescription
					.getTitle();
			boolean hasTitle = !TextUtils.isEmpty(title);

			CharSequence subtitle = mDescription == null ? null : mDescription
					.getSubtitle();
			...
			if (!hasTitle && !hasSubtitle) {
				...
				mSubtitleView.setVisibility(hasSubtitle ? View.VISIBLE
						: View.GONE);
			}
			if (mState != null) {
				boolean isPlaying = mState.getState() == PlaybackStateCompat.STATE_BUFFERING
						|| mState.getState() == PlaybackStateCompat.STATE_PLAYING;
				boolean supportsPlay = (mState.getActions() & (PlaybackStateCompat.ACTION_PLAY | PlaybackStateCompat.ACTION_PLAY_PAUSE)) != 0;
				boolean supportsPause = (mState.getActions() & (PlaybackStateCompat.ACTION_PAUSE | PlaybackStateCompat.ACTION_PLAY_PAUSE)) != 0;
	...
	private boolean isVolumeControlAvailable() {
		return mVolumeControlEnabled
				&& mRoute.getVolumeHandling() == MediaRouter.RouteInfo.PLAYBACK_VOLUME_VARIABLE;
	}
	...
	private final class MediaControllerCallback extends
