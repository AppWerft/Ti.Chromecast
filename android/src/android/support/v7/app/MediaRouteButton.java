 ...
 * allowed to select by specifying a {@link MediaRouteSelector selector} with
 * the {@link #setRouteSelector} method.
 * </p>
 * <p>
 * When the default route is selected or when the currently selected route does
 * not match the {@link #getRouteSelector() selector}, the button will appear in
 * an inactive state indicating that the application is not connected to a route
 * of the kind that it wants to use. Clicking on the button opens a
 * {@link MediaRouteChooserDialog} to allow the user to select a route. If no
 * non-default routes match the selector and it is not possible for an active
 * scan to discover any matching routes, then the button is disabled and cannot
 * be clicked.
 * </p>
 * <p>
 * When a non-default route is selected that matches the selector, the button
 * will appear in an active state indicating that the application is connected
 * to a route of the kind that it wants to use. The button may also appear in an
 * intermediary connecting state if the route is in the process of connecting to
 * the destination but has not yet completed doing so. In either case, clicking
 * on the button opens a {@link MediaRouteControllerDialog} to allow the user to
 * control or disconnect from the current route.
 * </p>
 *
 * <h3>Prerequisites</h3>
 * <p>
 * To use the media route button, the activity must be a subclass of
 * {@link FragmentActivity} from the <code>android.support.v4</code> support
 * library. Refer to support library documentation for details.
 * </p>
 *
...
public class MediaRouteButton extends View {
	...
	private MediaRouteDialogFactory mDialogFactory = MediaRouteDialogFactory
			.getDefault();
	...
	public MediaRouteButton(Context context, AttributeSet attrs,
			int defStyleAttr) {
		super(MediaRouterThemeHelper.createThemedContext(context), attrs,
				defStyleAttr);
		context = getContext();
	 ...
	 * chooser dialog. Otherwise, shows the route controller dialog to offer the
	 * user a choice to disconnect from the route or perform other control
	 * actions such as setting the route's volume.
	 * </p>
	 * <p>
	 * The application can customize the dialogs by calling
	 * {@link #setDialogFactory} to provide a customized dialog factory.
	 * </p>
	 *
	 * @return True if the dialog was actually shown.
	 *
	 * @throws IllegalStateException
	 *             if the activity is not a subclass of {@link FragmentActivity}
	 *             .
	 */
	...
	protected int[] onCreateDrawableState(int extraSpace) {
		...
		// are implementation details here. Checkable is used to express the
		// connecting
		// drawable state and it's mutually exclusive with check for the
		// purposes
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		final int minWidth = Math.max(mMinWidth,
				mRemoteIndicator != null ? mRemoteIndicator.getIntrinsicWidth()
						: 0);
		final int minHeight = Math.max(
				mMinHeight,
				mRemoteIndicator != null ? mRemoteIndicator
		...
		case MeasureSpec.AT_MOST:
			width = Math.min(widthSize, minWidth + getPaddingLeft()
					+ getPaddingRight());
			break;
		default:
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mRemoteIndicator != null) {
			...
			mRemoteIndicator.setBounds(drawLeft, drawTop, drawLeft + drawWidth,
					drawTop + drawHeight);
		...
		if (mAttachedToWindow) {
			...
			if (needsRefresh) {
				...
				if (mIsConnecting
					...
					AnimationDrawable curDrawable = (AnimationDrawable) mRemoteIndicator
							.getCurrent();
