 ...
 * MediaRouter for framework-provided routes and registers
 * non-framework-provided routes as user routes.
 */
abstract class SystemMediaRouteProvider extends MediaRouteProvider {
	...
	public static SystemMediaRouteProvider obtain(Context context,
	...
	/**
	 * Called by the media router when a route is added to synchronize state
	 * with the framework media router.
	 */
	public void onSyncRouteAdded(MediaRouter.RouteInfo route) {
	}

	/**
	 * Called by the media router when a route is removed to synchronize state
	 * with the framework media router.
	 */
	...
	/**
	 * Called by the media router when a route is changed to synchronize state
	 * with the framework media router.
	 */
	...
	/**
	 * Called by the media router when a route is selected to synchronize state
	 * with the framework media router.
	 */
	...
	/**
	 * Callbacks into the media router to synchronize state with the framework
	 * media router.
	 */
	...
	static class LegacyImpl extends SystemMediaRouteProvider {
		...
		public LegacyImpl(Context context) {
			...
			context.registerReceiver(mVolumeChangeReceiver, new IntentFilter(
					VolumeChangeReceiver.VOLUME_CHANGED_ACTION));
			publishRoutes();
		}
		...
		private void publishRoutes() {
			...
			mLastReportedVolume = mAudioManager
					.getStreamVolume(PLAYBACK_STREAM);
		...
		final class DefaultRouteController extends RouteController {
			...
			public void onUpdateVolume(int delta) {
				...
				int maxVolume = mAudioManager
						.getStreamMaxVolume(PLAYBACK_STREAM);
				int newVolume = Math
						.min(maxVolume, Math.max(0, volume + delta));
				if (newVolume != volume) {
					mAudioManager.setStreamVolume(PLAYBACK_STREAM, volume, 0);
				}
				publishRoutes();
			...
			public void onReceive(Context context, Intent intent) {
				if (intent.getAction().equals(VOLUME_CHANGED_ACTION)) {
					final int streamType = intent.getIntExtra(
							EXTRA_VOLUME_STREAM_TYPE, -1);
					if (streamType == PLAYBACK_STREAM) {
						final int volume = intent.getIntExtra(
								EXTRA_VOLUME_STREAM_VALUE, -1);
	 ...
	 */
	static class JellybeanImpl extends SystemMediaRouteProvider implements
			MediaRouterJellybean.Callback, MediaRouterJellybean.VolumeCallback {
		...
		// Maintains an association from framework routes to support library
		// routes.
		// Note that we cannot use the tag field for this because an application
		// may
		// have published its own user routes to the framework media router and
		// already
		...
		// Maintains an association from support library routes to framework
		// routes.
				...
				for (int i = 0; i < count; i++) {
					...
					if (category.equals(MediaControlIntent.CATEGORY_LIVE_AUDIO)) {
						newRouteTypes |= MediaRouterJellybean.ROUTE_TYPE_LIVE_AUDIO;
							...
							.equals(MediaControlIntent.CATEGORY_LIVE_VIDEO)) {
		...
		private String assignRouteId(Object routeObj) {
			// TODO: The framework media router should supply a unique route id
			// that
			// we can use here. For now we use a hash of the route name and take
			// care
			// to dedupe it.
			boolean isDefault = (getDefaultRoute() == routeObj);
			String id = isDefault ? DEFAULT_ROUTE_ID : String.format(Locale.US,
					"ROUTE_%08x", getRouteName(routeObj).hashCode());
		...
		public void onRouteVolumeChanged(Object routeObj) {
			if (getUserRouteRecord(routeObj) == null) {
				...
				if (index >= 0) {
					SystemRouteRecord record = mSystemRouteRecords.get(index);
					int newVolume = MediaRouterJellybean.RouteInfo
							.getVolume(routeObj);
					if (newVolume != record.mRouteDescriptor.getVolume()) {
						record.mRouteDescriptor = new MediaRouteDescriptor.Builder(
		...
		public void onRouteSelected(int type, Object routeObj) {
			if (routeObj != MediaRouterJellybean.getSelectedRoute(mRouterObj,
				...
				// The currently selected route has already changed so this
				// callback
			...
			if (userRouteRecord != null) {
				...
				// Select the route if it already exists in the compat media
				// router.
				// If not, we will select it instead when the route is added.
				int index = findSystemRouteRecord(routeObj);
				if (index >= 0) {
					SystemRouteRecord record = mSystemRouteRecords.get(index);
					MediaRouter.RouteInfo route = mSyncCallback
							.getSystemRouteByDescriptorId(record.mRouteDescriptorId);
					if (route != null) {
						route.select();
					}
				}
		...
		public void onSyncRouteAdded(MediaRouter.RouteInfo route) {
			if (route.getProviderInstance() != this) {
				...
				MediaRouterJellybean.UserRouteInfo.setVolumeCallback(routeObj,
						mVolumeCallbackObj);
				updateUserRouteProperties(record);
				mUserRouteRecords.add(record);
				MediaRouterJellybean.addUserRoute(mRouterObj, routeObj);
			} else {
				// If the newly added route is the counterpart of the currently
				// selected
				// route in the framework media router then ensure it is
				// selected in
				...
				if (index >= 0) {
					SystemRouteRecord record = mSystemRouteRecords.get(index);
					if (record.mRouteDescriptorId.equals(route
							.getDescriptorId())) {
						route.select();
					}
				}
		...
		public void onSyncRouteRemoved(MediaRouter.RouteInfo route) {
			if (route.getProviderInstance() != this) {
				...
				if (index >= 0) {
					...
					MediaRouterJellybean.RouteInfo.setTag(record.mRouteObj,
							null);
					MediaRouterJellybean.UserRouteInfo.setVolumeCallback(
							record.mRouteObj, null);
					MediaRouterJellybean.removeUserRoute(mRouterObj,
							record.mRouteObj);
				}
			}
		}
		...
		public void onSyncRouteSelected(MediaRouter.RouteInfo route) {
			...
			if (route.getProviderInstance() != this) {
				...
				int index = findSystemRouteRecordByDescriptorId(route
						.getDescriptorId());
				if (index >= 0) {
					SystemRouteRecord record = mSystemRouteRecords.get(index);
					selectRoute(record.mRouteObj);
		...
		protected UserRouteRecord getUserRouteRecord(Object routeObj) {
			...
			return tag instanceof UserRouteRecord ? (UserRouteRecord) tag
					: null;
		}

		protected void updateSystemRouteDescriptor(SystemRouteRecord record) {
			// We must always recreate the route descriptor when making any
			// changes
		...
		protected String getRouteName(Object routeObj) {
			// Routes should not have null names but it may happen for badly
			// configured
			// user routes. We tolerate this by using an empty name string here
			// but
			// such unnamed routes will be discarded by the media router
			// upstream
		...
		protected void onBuildSystemRouteDescriptor(SystemRouteRecord record,
			...
			int supportedTypes = MediaRouterJellybean.RouteInfo
					.getSupportedTypes(record.mRouteObj);
			...
			builder.setPlaybackType(MediaRouterJellybean.RouteInfo
					.getPlaybackType(record.mRouteObj));
			builder.setPlaybackStream(MediaRouterJellybean.RouteInfo
					.getPlaybackStream(record.mRouteObj));
			builder.setVolume(MediaRouterJellybean.RouteInfo
					.getVolume(record.mRouteObj));
			builder.setVolumeMax(MediaRouterJellybean.RouteInfo
					.getVolumeMax(record.mRouteObj));
			builder.setVolumeHandling(MediaRouterJellybean.RouteInfo
					.getVolumeHandling(record.mRouteObj));
		}

		protected void updateUserRouteProperties(UserRouteRecord record) {
			MediaRouterJellybean.UserRouteInfo.setName(record.mRouteObj,
					record.mRoute.getName());
			...
			MediaRouterJellybean.UserRouteInfo.setVolume(record.mRouteObj,
					record.mRoute.getVolume());
			MediaRouterJellybean.UserRouteInfo.setVolumeMax(record.mRouteObj,
					record.mRoute.getVolumeMax());
		...
		protected void updateCallback() {
			...
			if (mRouteTypes != 0) {
				...
				MediaRouterJellybean.addCallback(mRouterObj, mRouteTypes,
						mCallbackObj);
			}
		}
		...
		/**
		 * Represents a route that is provided by the framework media router and
		 * published by this route provider to the support library media router.
		 */
		protected static final class SystemRouteRecord {
			public final Object mRouteObj;
			public final String mRouteDescriptorId;
			public MediaRouteDescriptor mRouteDescriptor; // assigned
															// immediately after
															// creation
		...
		/**
		 * Represents a route that is provided by the support library media
		 * router and published by this route provider to the framework media
		 * router.
		 */
		...
		protected final class SystemRouteController extends RouteController {
			...
			@Override
			public void onUpdateVolume(int delta) {
				MediaRouterJellybean.RouteInfo.requestUpdateVolume(mRouteObj,
						delta);
			}
		}
	}
	 ...
	 */
	private static class JellybeanMr1Impl extends JellybeanImpl implements
		...
		public void onRoutePresentationDisplayChanged(Object routeObj) {
			...
			if (index >= 0) {
				SystemRouteRecord record = mSystemRouteRecords.get(index);
				Display newPresentationDisplay = MediaRouterJellybeanMr1.RouteInfo
						.getPresentationDisplay(routeObj);
				int newPresentationDisplayId = (newPresentationDisplay != null ? newPresentationDisplay
						.getDisplayId() : -1);
				if (newPresentationDisplayId != record.mRouteDescriptor
						.getPresentationDisplayId()) {
					record.mRouteDescriptor = new MediaRouteDescriptor.Builder(
							record.mRouteDescriptor).setPresentationDisplayId(
							newPresentationDisplayId).build();
					publishRoutes();
				}
			}
		}
		...
		protected void onBuildSystemRouteDescriptor(SystemRouteRecord record,
			...
			Display presentationDisplay = MediaRouterJellybeanMr1.RouteInfo
					.getPresentationDisplay(record.mRouteObj);
			if (presentationDisplay != null) {
				builder.setPresentationDisplayId(presentationDisplay
						.getDisplayId());
			}
		}
		...
		protected void updateCallback() {
			...
			mActiveScanWorkaround
					.setActiveScanRouteTypes(mActiveScan ? mRouteTypes : 0);
		}
	...
	private static class JellybeanMr2Impl extends JellybeanMr1Impl {
		...
		protected void onBuildSystemRouteDescriptor(SystemRouteRecord record,
			...
			CharSequence description = MediaRouterJellybeanMr2.RouteInfo
					.getDescription(record.mRouteObj);
			if (description != null) {
			...
			MediaRouterJellybeanMr2
					.addCallback(
							...
							MediaRouter.CALLBACK_FLAG_UNFILTERED_EVENTS
									| (mActiveScan ? MediaRouter.CALLBACK_FLAG_PERFORM_ACTIVE_SCAN
											: 0));
		}

		@Override
		protected boolean isConnecting(SystemRouteRecord record) {
			return MediaRouterJellybeanMr2.RouteInfo
					.isConnecting(record.mRouteObj);
		}
	}
}
